...
// Raw occupancy rate history data from API
export type OccupancyRateHistory = {
  leasing: {
...
// Component props
export interface OccupancyRateChartProps {
  occupancyRateHistories: OccupancyRateHistory[];
  height?: number;
  compact?: boolean;
  className?: string;
}
...
const ChartLegend = memo(() => (
  <div className="flex justify-center w-full mb-2">
    <div className="inline-flex items-center gap-2">
      <div className="w-3 h-3 rounded-full" style={{ backgroundColor: CHART_COLORS.occupancyRate }}></div>
      <span className="text-sm">稼働率</span>
    </div>
  </div>
));
...
function OccupancyRateChart({
  ...
  const sortedHistories = useMemo(() => {
    if (!occupancyRateHistories?.length) return [];
    
    return [...occupancyRateHistories].sort((a, b) => 
  ...
  const chartData = useMemo(() => {
    ...
    const groupedData: Record<string, { points: OccupancyRateHistory[]; avgRate: number }> = {};
    ...
    sortedHistories.forEach(point => {
      ...
      if (!groupedData[groupKey]) {
        groupedData[groupKey] = { points: [], avgRate: 0 };
      }
    ...
    // Calculate average occupancy rate for each group and select representative data points
    const resultPoints: ChartDataPoint[] = Object.keys(groupedData).map(groupKey => {
      const group = groupedData[groupKey];
      
      // Calculate average occupancy rate
      const sum = group.points.reduce((total, point) => {
        return total + parseFloat(point.leasing.occupancyRate || '0');
      }, 0);
      group.avgRate = parseFloat((sum / group.points.length).toFixed(2));
  ...
  return (
    <div className={`bg-white p-4 rounded-lg shadow ${className || ''} ${compact ? 'p-2' : ''}`}>
      <div className="flex items-center justify-start mb-4">
        <h3 className={`text-lg font-semibold ${compact ? 'text-base' : ''}`}>稼働率</h3>
      </div>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <Line
                      ...
                      name="稼働率"
                      stroke={CHART_COLORS.occupancyRate}
                      strokeWidth={2}
                      activeDot={{ r: 6 }}
                      dot={{ r: 4 }}
                    />
...
export default memo(OccupancyRateChart); 